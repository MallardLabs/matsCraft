...
const scoreBoardConfig = (player) => {
    ...
    data.push(`§fName: §f${PlayerName}`, `§fDiscord: §9${Discord}`, `§fMats: §e${Mats}`, `§fHuh: §f${Huh}`, `§fLevel: §f${PlayerLevel}`, `§fOnline: §a${OnlinePlayers} §f/ §a1000`);
    return data.join("\n");
};
function displayPlayerScores() {
    system.run(() => {
        for (const player of world.getPlayers()) {
            if (player.hasTag("hide-scoreboard"))
                return;
