...
class BlockManager {
    static handleBlockBreak(data, actions) {
        const { player, blockId, location, toolTypeId, dimension } = data;
        if (!this.isValidBlock(blockId, toolTypeId) ||
    ...
    static getValidPickaxe(toolTypeId, blockId) {
        const pickAxe = pickAxeAbility.find((p) => p.typeId === toolTypeId);
        return pickAxe?.allowed.includes(blockId) ? pickAxe : null;
    }
    static processBlockBreak(blockId, location, player, toolTypeId, actions) {
        const xuid = player.getDynamicProperty("xuid");
        const drop = ore_config[blockId.replace(this.MATSCRAFT_PREFIX, "")];
        actions.dropItem(drop);
        this.storePendingBlock(blockId, location, xuid, toolTypeId);
    }
    static storePendingBlock(blockId, location, xuid, toolTypeId) {
        system.run(() => {
    ...
    static getPendingBlocks() {
        const rawData = world.getDynamicProperty("pendingBlock");
        return rawData ? JSON.parse(rawData) : [];
    }
    static createBlockData(xuid, blockId, location, toolTypeId) {
        return {
    ...
    static async sendBlocksToServer(data) {
