...
itemPickup.listen(async (data, actions) => {
  ...
  if (!playerData) {
    if (typeId === "matsphone:matsphone") return;
  ...
  if (typeId !== "matscraft:mats") return;
...
const getPendingUpdate = (): PendingUpdate[] => {
  const raw = world.getDynamicProperty("pendingItemUpdate") as string;
  if (raw) {
    try {
...
const updatePendingUpdate = (data: PendingUpdate[]) => {
